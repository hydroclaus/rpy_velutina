...
class SCD30:
    ...
    def start_continous_measurement(self, ambient_pressure=0):
        self.__write_command_argument(self.START_CONT_MEASURE, ambient_pressure)
    ...
    def set_measurement_interval(self, interval):
        self.__write_command_argument(self.SET_MEASURE_INTERVAL, interval)
    ...
    def set_automatic_recalibration(self, enable):
        self.__write_command_argument(self.SET_ASC, 1 if enable else 0)
    ...
    def set_forced_recalibration(self, co2ppm):
        self.__write_command_argument(self.SET_FRC, co2ppm)
    ...
    def set_temperature_offset(self, offset):
        self.__write_command_argument(self.SET_TEMP_OFFSET, int(offset * 100))
    ...
    def set_altitude_comp(self, altitude):
        self.__write_command_argument(self.SET_ALT_COMP, altitude)

    def __write_command(self, cmd):
        bcmd = struct.pack('>H', cmd)
        self.i2c.writeto(self.addr, bcmd)

    def __write_command_argument(self, cmd, value):
        bcmd = struct.pack('>H', cmd)
        bval = struct.pack('>H', value)
        crc = self.__crc(bval[0], bval[1])
        self.i2c.writeto(self.addr, bcmd + bval + bytes([crc]))
